...
public class DerivativeStructureSymbol<T, A>(
    ...
    symbol: Symbol,
    value: T,
) : DerivativeStructure<T, A>(derivativeAlgebra, size, order, index, value),
    Symbol where A : Ring<T>, A : NumericAlgebra<T>, A : ScaleOperations<T> {
...
public open class DerivativeStructureRing<T, A>(
     ...
     */
    private val variables: Map<String, DerivativeStructureSymbol<T, A>> =
        bindings.entries.mapIndexed { index, (key, value) ->
            key.identity to DerivativeStructureSymbol(
                this,
                numberOfVariables,
                order,
                index,
    ...
    override fun bindSymbolOrNull(value: String): DerivativeStructureSymbol<T, A>? = variables[value]
    override fun bindSymbol(value: String): DerivativeStructureSymbol<T, A> = variables.getValue(value)

    public fun bindSymbolOrNull(symbol: Symbol): DerivativeStructureSymbol<T, A>? = variables[symbol.identity]
    public fun bindSymbol(symbol: Symbol): DerivativeStructureSymbol<T, A> = variables.getValue(symbol.identity)
    ...
    override fun multiply(left: DerivativeStructure<T, A>, right: DerivativeStructure<T, A>): DerivativeStructure<T, A> {
        left.compiler.checkCompatibility(right.compiler)
    ...
    override operator fun DerivativeStructure<T, A>.minus(other: Number): DerivativeStructure<T, A> = this + -other.toDouble()
...
public class DerivativeStructureField<T, A : ExtendedField<T>>(
    ...
    order: Int,
    bindings: Map<Symbol, T>,
) : DerivativeStructureRing<T, A>(algebra, bufferFactory, order, bindings),
    ExtendedField<DerivativeStructure<T, A>> {
    override fun number(value: Number): DerivativeStructure<T, A> = const(algebra.number(value))
